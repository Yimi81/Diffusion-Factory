...
from diffusers import AutoencoderKL, DDPMScheduler, StableDiffusionControlNetPipeline, UNet2DConditionModel, ControlNetModel, DiffusionPipeline, UniPCMultistepScheduler
...
class CustomControlNetTrainer:
    ...
    def train(self,
        ...
        if accelerator.is_main_process:
            ...
            controlnet.save_pretrained(self.training_args.output_dir)
    ...
    def validation(self, is_test=False):
        ...
        validation_image = Image.open(self.training_args.validation_image).convert("RGB")
        for i in range(self.training_args.num_validation_images):
            with torch.autocast("cuda"):
                image = pipeline(self.training_args.validation_prompt, validation_image, num_inference_steps=20, generator=generator).images[0]
        ...
        for tracker in self.accelerator.trackers:
            ...
            if tracker.name == "wandb":
                ...
                for i, image in enumerate(images):
                    image = wandb.Image(image, caption=f"{i}: {self.training_args.validation_prompt}")
                    formatted_images.append(image)

                tracker.log({"test" if is_test else "validation": formatted_images})
            else:
